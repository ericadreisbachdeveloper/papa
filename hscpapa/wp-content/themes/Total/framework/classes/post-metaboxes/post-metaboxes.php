...
if ( ! class_exists( 'WPEX_Post_Metaboxes' ) ) {
	...
	class WPEX_Post_Metaboxes {
		...
		public function scripts() {
			...
			$dir = WPEX_FRAMEWORK_DIR_URI .'classes/post-metaboxes/assets/';
			...
			foreach ( $settings as $setting ) {
				...
				if ( isset( $_POST[$id] ) ) {
					...
					if ( 'media' == $type ) {
						...
						if ( 'wpex_post_self_hosted_media' == $id && empty( $_POST[$id] ) && $old = get_post_meta( get_the_ID(), 'wpex_post_self_hosted_shortcode_redux', true ) ) {
							$value = $old;
							delete_post_meta( get_the_ID(), 'wpex_post_self_hosted_shortcode_redux' );
						}
