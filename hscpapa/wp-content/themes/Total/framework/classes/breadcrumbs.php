...
function wpex_has_breadcrumbs( $post_id = '' ) {
	...
	// Check page settings
	if ( $meta = get_post_meta( $post_id, 'wpex_disable_breadcrumbs', true ) ) {
...
function wpex_display_breadcrumbs() {
	...
	// Built-in breadcrumbs
		...
		echo wpex_breadcrumbs( $obj->post_id );
	}
...
if ( ! function_exists( 'wpex_breadcrumbs' ) ) {
	function wpex_breadcrumbs( $post_id = '' ) {
		...
		/*-----------------------------------------------------------------------------------*/
		if ( ! is_front_page() ) {
			if ( $home_title = wpex_get_mod( 'breadcrumbs_home_title' ) ) {
		...
		elseif ( is_singular() ) {
			...
			if ( 'post' == $post_type ) {
				...
				// Categories
				if ( wpex_get_mod( 'breadcrumbs_blog_cat', true ) && $terms = wpex_list_post_terms( $taxonomy = 'category', $show_links = true, $echo = false ) ) {
			...
			if ( $post_type == 'portfolio' ) {
				...
				if ( $portfolio_page = wpex_get_mod( 'portfolio_page' ) ) {
				...
				if ( wpex_get_mod( 'breadcrumbs_portfolio_cat', true )
			...
			if ( $post_type == 'staff' ) {
				...
				// Display staff page
				if ( $staff_page = wpex_get_mod( 'staff_page' ) ) {
				...
				// Staff Categories
				if ( wpex_get_mod( 'breadcrumbs_staff_cat', true )
			...
			if ( $post_type == 'testimonials' ) {
				...
				if ( $testimonials_page = wpex_get_mod( 'testimonials_page' ) ) {
				...
				if ( wpex_get_mod( 'breadcrumbs_testimonials_cat', true )
			...
			if ( ! empty( $post_title ) ) {
				if ( $trim_title        = wpex_get_mod( 'breadcrumbs_title_trim', '4' ) ) {
