...
function vcex_no_posts_found_message( $atts ) {
	...
	if ( ! wpex_is_front_end_composer() || ( ! wpex_is_front_end_composer() && 'tribe_events' != $post_type ) ) {
		return;
	}
...
if ( ! function_exists( 'vcex_row_video' ) ) {
	function vcex_row_video( $atts ) {
		...
		if ( 'self_hosted' == $video_bg ) { ?>
			...
			<video class="wpex-video-bg" poster="<?php echo wp_get_attachment_url( $bg_image ); ?>" preload="auto" autoplay="true" loop="loop" muted volume="0">
		...
		elseif ( 'youtube' == $video_bg && ! empty( $video_bg_youtube ) ) {
			...
			<embed class="wpex-video-bg" src="<?php echo $video_bg_youtube; ?>?loop=1&amp;autoplay=1&amp;controls=0&amp;showinfo=0&amp;autohide=1&amp;sound=0" width="100%" height="100%"></embed>
...
if ( ! function_exists( 'vcex_parallax_bg' ) ) {
	...
	function vcex_parallax_bg( $atts ) {
		...
		// Classes
		$classes = array( 'wpex-parallax-bg', 'bg-cover' );
