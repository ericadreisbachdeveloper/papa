...
class WPEX_Visual_Composer {
	...
	public function __construct() {
		...
		$this->vc_theme_mode = get_theme_mod( 'visual_composer_theme_mode', true );
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'vc_stylesheets' ) );

		// Apply filters so you can disable modifications via a child theme
		self::$has_vc_edits = apply_filters( 'wpex_edit_visual_composer', self::$has_vc_edits );
		...
		require_once( WPEX_FRAMEWORK_DIR .'visual-composer/helpers/parse-row-atts.php' );
	...
	public function global_object() {
		...
		foreach ( $get_terms as $term ) {
			$group = isset( $taxonomies[$term->taxonomy]->labels ) ? $taxonomies[$term->taxonomy]->labels->name : __( 'Taxonomies', 'wpex' );
			$vcex_global->terms[] = array(
				'label'     => $term->name,
				'value'     => $term->slug,
		...
		// Store categories
		$get_terms               = get_terms( 'category', array( 'hide_empty' => false ) );
		$vcex_global->categories = array();
		foreach ( $get_terms as $term ) {
			$vcex_global->categories[] = array(
				'label' => $term->name,
				'value' => $term->term_id,
	 ...
	 * @access public
	 */
	public function vc_stylesheets() {
		wp_enqueue_style( 'wpex-visual-composer', WPEX_CSS_DIR_URI .'wpex-visual-composer.css', array( 'wpex-style', 'js_composer_front' ), WPEX_THEME_VERSION );
		wp_enqueue_style( 'wpex-visual-composer-extend', WPEX_CSS_DIR_URI .'wpex-visual-composer-extend.css', array( 'wpex-style', 'js_composer_front' ), WPEX_THEME_VERSION );
	}
	...
	/**
	 * Display outdated notice if the VC version active needs updating
	 *
	 * @since  2.1.0
	 * @access public
	 *
	...
	public function outdated_notice() {
		...
		// If current version is smaller then supported version set display to true
		if ( defined( 'WPB_VC_VERSION' ) && ( WPB_VC_VERSION < WPEX_VC_SUPPORTED_VERSION ) ) {
			$display = true;
		}

		// Apply filters for child theming
		$display = apply_filters( 'wpex_vc_update_notice', $display );

		// Display notice if display is true
		if ( $display ) {

        	echo '<div class="update-nag error">
        		<p>'. __( 'This theme version requires an updated Visual Composer plugin. Please follow the theme documentation to update it.', 'wpex' ) .'</p>
        		<p><strong><a href="http://wpexplorer-themes.com/total/docs/updating-visual-composer/" target="_blank">'. __( 'Learn More', 'wpex' ) .' &rarr;</a></strong></p>
        	</div>';
        }
