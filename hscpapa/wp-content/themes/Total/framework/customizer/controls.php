...
class WPEX_Fonts_Dropdown_Custom_Control extends WP_Customize_Control {
	...
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			...
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php _e( 'Default', 'wpex' ); ?></option>
			<?php
			// Add custom fonts from child themes
			if ( function_exists( 'wpex_add_custom_fonts' ) ) {
				$fonts = wpex_add_custom_fonts();
				if ( $fonts && is_array( $fonts ) ) { ?>
					<optgroup label="<?php _e( 'Custom Fonts', 'wpex' ); ?>">
			...
			<optgroup label="<?php _e( 'Standard Fonts', 'wpex' ); ?>">
				<?php
				// Get font options
			...
			<optgroup label="<?php _e( 'Google Fonts', 'wpex' ); ?>">
				<?php
				// Get font options
